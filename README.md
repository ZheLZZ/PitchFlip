...
![PitchFlip 顶部装订展开预览：上方为背面，下方为下一张纸正面](docs/images/v0.3.0-desktop-preview.jpg)

上方显示刚翻过去那张纸的背面，下方显示下一张纸的正面。左侧按观看视角组织为**封面、2–3 页、4–5 页……封底**，每组标明上页和下页，与中央预览一致。右侧同时标明**原 PDF 页码、输出页码、物理纸张和正反面**。首图为 v0.3.0 实际桌面窗口截图，其余截图展示新版预览工作区，使用仓库自带的合成测试 PDF。
...
- **同步定位**：左侧按上下页组合分组，跟随当前页高亮、滚动，支持折叠分组。
- **预览缩放**：底部可调整缩放比例，或点击“适合窗口”恢复完整视图。
- **撤销与重做**：可删除用户插入的空白，不能删除原始 PDF 页面。
- **保存副本**：复制原始 PDF 页面内容，不将原页重新栅格化，也不覆盖原文件。

![插入空白后：原 PDF 第 5 页后移，输出文件由 6 页变成 7 页](docs/images/v0.3.0-insert-blank.png)
...
选中原 PDF 页面后，点击工具栏或右键菜单中的“独占一张纸”。页面在正面时补页后空白；在背面时补页前和页后空白，使该页位于正面、背面留空。已有页后空白会复用，整个操作可一次撤销。翻页后，左侧导航默认选中预览上半部分的页面；封面选中第一页。

### 页面总览

点击“页面总览”可多列查看顶部装订的上下双页组合（封面、2–3 页、4–5 页……封底），与翻页预览一致，滚动浏览并调整缩略图大小。总览标明正反面及空白页，点击页面返回对应翻页预览，右键可调整空白页；按 Esc 返回翻页。

![新版页面总览：上下双页组合与插入空白的位置](docs/images/v0.3.0-page-overview.png)
